...
A_H2O = -3385.59633521043;  B_H2O = 44.119996293;
C_H2O = -0.22135593182;     D_H2O = 0.00054657156138;
E_H2O = -6.6489932771e-007; F_H2O = 3.1947490101e-010;
Cp_H2O = A_H2O + B_H2O*Tm_H2O + C_H2O*Tm_H2O^2 + D_H2O*Tm_H2O^3 + E_H2O*Tm_H2O^4 + F_H2O*Tm_H2O^5; % J/molK

% Conductivité
k_H2O = -0.298749014550685 + 4.70536755156601E-03*Tm_H2O + -5.62090309000001E-06*Tm_H2O^2; % W/mK
...
R_ = (Thi - Tho)/(Tco - Tci);
P_ = (Tco - Tci)/(Thi - Tci);
Pz = (1-((P_*R_-1)/(P_-1))^(1/N))/(R_-((P_*R_ - 1)/(P_ - 1))^(1/N));
S = sqrt(R_^2 + 1)/(R_ - 1);
F = S*log((1-Pz)/(1-R_*Pz))/log((2/Pz-1-R+sqrt(R^2+1))/(2/Pz-1-R-sqrt(R^2+1)));
...
while abs(Ucal - U) > tol
    ...
    Pr = mu_gaz*Cp_gaz/(M_gaz*k_gaz);
    Nu = 0.023*Re^0.8*Pr^0.33; % (Nitsche & Gbadamosi, 2016, p. 39)
    hi = Nu*k_gaz/Dti;
